...
fn generate_nn_module(size: usize) -> String {
    ...
    // temp variables for sign activation
    for l in 0..(NUM_LAYERS-1) {
        for r in 0..OUT_NEURONS[l] {
            module_code.push_str(&format!("  wire [15:0] t_sign_l{}_r{};\n", l, r));
        }
    }
        ...
        for r in 0..OUT_NEURONS[l] {
            for c in 0..in_size {
                if l == 0 {
                    ...
                    module_code.push_str(&format!(
                        "  assign t0_l{}_r{}_c{} = weights_{}_{}_{} * t_sign_l{}_r{};\n",
                        l, r, c, l, c, r, l - 1, c
                    ));
                }
            }
            ...
            if l < (NUM_LAYERS - 1) {
                module_code.push_str(&format!(
                        "  assign t_sign_l{}_r{} = {{1'b0, 1'b0, 1'b0, 1'b0, 1'b0, 1'b0, 1'b0, 1'b0, 1'b0, 1'b0, 1'b0, 1'b0, 1'b0, 1'b0, 1'b0, t{}_l{}_r{}_c{}[15]}};\n",
                        l, r, log_size, l, r, 0
                ));
            } else {
                module_code.push_str(&format!(
